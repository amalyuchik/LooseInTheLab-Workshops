...
/*********************Choose the season form variables*******************/
...
$my_query_workshop_id = "SELECT * FROM workshop WHERE year = $year AND season = $season AND cancelled != 1 ORDER BY wdate, city, state ASC ";
...
$next_year = $year + 1;
	...
	echo '<p><a href="';
	echo 'http://www.seriouslyfunnyscience.com/workshops/workshop-info-chrono.php?season=' . "$next_season" . '&year=' . "$next_year" . '&Choose=Submit';
	echo '">Next Season</a></p>';
	echo '<p><a href="';
	echo "http://www.seriouslyfunnyscience.com/workshops/workshop-info-chrono.php";
	echo '">Current Season</a></p>';

	echo '<p><a href="';
	echo 'http://www.seriouslyfunnyscience.com/workshops/workshop-info-chrono.php?season=' . "$last_season" . '&year=' . "$last_year" . '&Choose=Submit';
	echo '">Previous Season</a></p>';
echo '<form id=selfrmtxt name="form_city" method="GET" action="workshop_db.php">';
echo '<p class="noPrint">Sort By:</p><input class="noPrint" name="order" type="radio" value="last_name" checked /><label class="noPrint">Name</label><br /><input class="noPrint" name="order" type="radio" value="school" /><label class="noPrint">School</label><hr class="noPrint" />';
...
echo $avg_round_off;
echo " </strong> </p>";
	echo '<br /><br /><input style="float: none; " class="button" type="submit" name="submit" value="Go" />';
	echo '</form></div>';
//include($_SERVER['DOCUMENT_ROOT'].'/new_out.inc');
/*<input type=\"hidden\" name=\"state_table\" value=\"" . "$row[state]" . "\" />*/?>
